<template>
  <el-form label-width="100px">
    <el-form-item label="SKU名称">
      <el-input type="primary" placeholder="SKU名称" v-model="skuParams.skuName"></el-input>
    </el-form-item>
      ...
      <el-form :inline="true">
        <el-form-item v-for="(item,index) in saleArr" :key="item.id" :label="item.saleAttrName">
          <el-select v-model="item.saleIdAndValueId" style="width: 150px">
            <el-option :value="`${item.id}:${saleAttrValue.id}`" v-for="(saleAttrValue,index) in item.spuSaleAttrValueList" :label="saleAttrValue.saleAttrValueName"></el-option>
          </el-select>
        </el-form-item>        
      </el-form>
    </el-form-item>
...
let saleArr = ref<any>([])
let imgArr = ref<any>([])
let skuParams = reactive<SkuData>({//父组件传递过来的数据
    "category3Id": "",//三级分类的ID
    "spuId": "",//已有的SPU的ID
    "tmId": "",//SPU品牌的ID
    //v-model收集
